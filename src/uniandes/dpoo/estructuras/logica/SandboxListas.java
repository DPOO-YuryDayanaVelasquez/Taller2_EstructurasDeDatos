...
public class SandboxListas
{
    ...
    {
    	ArrayList<Integer> listaEnterosNueva = new ArrayList<>();
    	for(int i=0; i<valores.length; i++) {
    		int valor = (int) valores[i];
    		listaEnterosNueva.add(valor);
    	}
    	
    	listaEnteros = listaEnterosNueva;
    }
    ...
    {
    	LinkedList<String> listaCadenasNueva = new LinkedList<>();
    	for(Object o : objetos) {
    		String objeto = o.toString();
    		listaCadenasNueva.add(objeto);
    	}
    	listaCadenas = listaCadenasNueva;
    }
    ...
    public int contarEnterosRepetidos( )
    {
        ...
        ArrayList<Integer> listaRecorridos = new ArrayList<>();
        for (int i=0;  i<listaEnteros.size(); i++) {
        	if (!listaRecorridos.contains(listaEnteros.get(i))) {
        		listaRecorridos.add(listaEnteros.get(i));
        		int aparacionesNumero = 0;
            	for (int j : listaEnteros) {
            		if (listaEnteros.get(i)==j) {
            			aparacionesNumero+=1;
            		}
            	}
            	if (aparacionesNumero>1) {
            		cantidadRepetidos +=1;
            	}
        	} 
        }
    ...
    {
    	ArrayList<Integer> listaEnterosRandom = new ArrayList<>(cantidad);
    	for (int i=0; i<cantidad; i++) {
    		int random = (int) (Math.random() * (maximo - minimo +1) + minimo);
    		listaEnterosRandom.add(random); 
    	}
    	
    	listaEnteros = listaEnterosRandom;
    }
